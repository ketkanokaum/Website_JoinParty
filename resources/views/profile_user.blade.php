...
<div class="bg">
  <div class="profile-card">
    ...
    <img src="{{ $user->images ? asset('storage/' . $user->images) : asset('images/user-default.png') }}" class="profile-image" alt="User Profile Image">
    <div class="profile-info">
      <h2>{{$user->username}}</h2>
      <p><b>{{ $user->fristname . ' ' . $user->lastname }}</b></p>
      <p><b>อีเมล : {{$user->email}} </b></p>
      <p><b>เพศ : {{$user->gender ?? '-' }}</b></p>
      <p><b>วันเกิด : {{ thaidate($user->birthday ?? '-') }}</b></p>
    ...
    <div class="buttons">
      <button type="button" class="button" data-bs-toggle="modal" data-bs-target="#exampleModal{{$user->id}}" data-bs-whatever="@mdo" style="background-color:#4e96e88a;">
        <img src="" alt="">แก้ไขโปรไฟล์
      </button>
      <form method="POST" action="{{ route('logout') }}">
        @csrf
        <button type="submit" style="background-color: rgba(208, 70, 70, 0.706);">
